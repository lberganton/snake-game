...
int interfaceMenu(const char *title, const int n, ...) {
  ...
  do {
    if (LINES < n + 4 || COLS < MENU_WIDTH) {
      handleWithSmallTermSize(n + 4, MENU_WIDTH);
    }

    va_list args;
    va_start(args, n);

    WINDOW* header = newwin(2, MENU_WIDTH, (LINES - (n + 4)) / 2, (COLS - MENU_WIDTH) / 2);
    WINDOW* options = newwin(n + 2, MENU_WIDTH, (LINES - n) / 2, (COLS - MENU_WIDTH) / 2);
    ...
    wmove(header, 1, (MENU_WIDTH - strlen(title)) / 2);
    ...
    wmove(options, pos, 1);
    wchgat(options, MENU_WIDTH - 2, A_REVERSE, 0, NULL);
    ...
    while ((input = wgetch(options)) != '\n' && input != KEY_RESIZE) {
      ...
      case 'W':
      case KEY_UP:
        wchgat(options, MENU_WIDTH - 2, A_NORMAL, 0, NULL);
